...
use babyjubjub::{edwards, FixedGenerators, JubjubEngine, JubjubParams};
    ...
    .enumerate()
  {
    ...
    if result.is_none() {
      ...
      result = Some(
        result
          .unwrap()
          .add(cs.namespace(|| format!("addition {}", i)), &p, params)?,
      );
    }
  }
...
mod test {
  ...
  use babyjubjub::{edwards, montgomery, FixedGenerators, JubjubBn256, JubjubParams};
  ...
  fn test_conditionally_select() {
    ...
    for _ in 0..1000 {
      ...
      let mut b = if rng.gen() {
        Boolean::from(
          AllocatedBit::alloc(cs.namespace(|| "condition"), Some(should_we_select)).unwrap(),
        )
      } else {
