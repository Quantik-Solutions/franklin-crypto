...
impl<E: Engine> Expression<E> {
  ...
  where
    CS: ConstraintSystem<E>,
  {
    pub fn kary_and<E, CS>(mut cs: CS, v: &[AllocatedBit]) -> Result<AllocatedBit, SynthesisError>
    where
      E: Engine,
      CS: ConstraintSystem<E>,
    {
  ...
  where
    CS: ConstraintSystem<E>,
  {
    let value = AllocatedNum::alloc(cs.namespace(|| "allocate number"), || self.value.grab())?;
...
mod test {
  ...
  fn test_expr_conditional_reversal() {
    let mut rng = XorShiftRng::from_seed([0x3dbe6259, 0x8d313d76, 0x3237db17, 0xe5bc0654]);
    {
      ...
      let (c, d) =
        Expression::conditionally_reverse(cs.namespace(|| "reverse1"), &a, &b, &condition).unwrap();
  ...
  fn test_expr_conditional_select() {
    let mut rng = XorShiftRng::from_seed([0x3dbe6259, 0x8d313d76, 0x3237db17, 0xe5bc0654]);
    {
      ...
      let c =
        Expression::conditionally_select(cs.namespace(|| "c"), &a, &b, &condition_true).unwrap();

      let condition_false = Boolean::constant(false);
      let d =
        Expression::conditionally_select(cs.namespace(|| "d"), &a, &b, &condition_false).unwrap();
