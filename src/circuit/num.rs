...
impl<E: Engine> AllocatedNum<E> {
  ...
  pub fn add_constant<CS>(&self, mut cs: CS, constant: E::Fr) -> Result<Self, SynthesisError>
  where
    CS: ConstraintSystem<E>,
  {
    let mut value = None;
  ...
  pub fn equals<CS>(mut cs: CS, a: &Self, b: &Self) -> Result<boolean::AllocatedBit, SynthesisError>
  where
    E: Engine,
    CS: ConstraintSystem<E>,
  {
...
mod test {
  ...
  fn test_num_conditional_select() {
    let mut rng = XorShiftRng::from_seed([0x3dbe6259, 0x8d313d76, 0x3237db17, 0xe5bc0654]);
    {
      ...
      let c =
        AllocatedNum::conditionally_select(cs.namespace(|| "c"), &a, &b, &condition_true).unwrap();

      let condition_false = Boolean::constant(false);
      let d =
        AllocatedNum::conditionally_select(cs.namespace(|| "d"), &a, &b, &condition_false).unwrap();
