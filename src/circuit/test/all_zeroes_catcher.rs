...
fn find<E: Engine>(aux: &[Vec<(E::Fr, usize)>]) -> HashSet<usize> {
  let zero = E::Fr::zero();
...
fn find_in_multiple_only<E: Engine>(aux: &[Vec<(E::Fr, usize)>]) -> HashSet<usize> {
  let zero = E::Fr::zero();
  let mut result = HashSet::new();
  // iterate over all columns
  for (idx, var) in aux.iter().enumerate() {
...
impl<E: Engine> ZeroCatchingTestConstraintSystem<E> {
  ...
  pub fn check_for_zero_columns_inputs(&self) -> HashSet<usize> {
    ...
    let result: HashSet<usize> = result
      .intersection(&find::<E>(&self.bt_inputs))
      .map(|el| *el)
      .collect();
    let result: HashSet<usize> = result
      .intersection(&find::<E>(&self.ct_inputs))
      .map(|el| *el)
      .collect();
  ...
  pub fn check_for_zero_columns_aux(&self) -> HashSet<usize> {
    ...
    let result: HashSet<usize> = result
      .intersection(&find::<E>(&self.bt_aux))
      .map(|el| *el)
      .collect();
    let result: HashSet<usize> = result
      .intersection(&find::<E>(&self.ct_aux))
      .map(|el| *el)
      .collect();
