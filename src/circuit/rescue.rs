  ...
  CS: ConstraintSystem<E>,
{
  ...
  for round_num in 0..(2 * params.num_rounds()) {
    ...
    for row_idx in 0..state_len {
      ...
      let with_round_constant = linear_applied.add_constant(CS::one(), round_constants[row_idx]);
...
impl<E: RescueEngine> StatefulRescueGadget<E> {
  ...
  pub fn specialize<CS: ConstraintSystem<E>>(&mut self, cs: CS, dst: u8) {
    assert!(dst > 0);
    let dst_as_fe = {
      let mut repr = <E::Fr as PrimeField>::Repr::default();
    ...
    match self.mode {
      RescueOpMode::AccumulatingToAbsorb(ref into) => assert_eq!(
        into.len(),
        0,
    ...
    match self.mode {
      RescueOpMode::AccumulatingToAbsorb(ref mut into) => {
        ...
        let output = sponge_output
          ...
          .next()
          .unwrap()
          .into_allocated_num(cs.namespace(|| "transform sponge output into allocated number"))?;
  ...
  fn test_rescue_mimc_gadget() {
...

    {
      ...
      let input_words: Vec<Num<Bn256>> = input
        ...
        .enumerate()
        .map(|(i, b)| {
          let v = AllocatedNum::alloc(cs.namespace(|| format!("input {}", i)), || Ok(*b)).unwrap();
      ...
      let res =
        rescue_mimc_over_lcs(cs.namespace(|| "rescue mimc"), &input_words, &params).unwrap();
      ...
      fn synthesize<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        for _ in 0..self.num_duplicates {
          ...
          for (i, inp) in self.input.iter().enumerate() {
            let v = AllocatedNum::alloc(cs.namespace(|| format!("hash input {}", i)), || Ok(*inp))?;

            input_words.push(v);
          }

          let mut res = rescue_hash(cs.namespace(|| "rescue hash"), &input_words, &self.params)?;
