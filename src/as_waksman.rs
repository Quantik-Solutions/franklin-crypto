    ...
    if num_columns > columns_to_generate {
      ...
      if rows_to_generate % 2 == 1 {
        ...
        if permutation.get(high) == high {
          ...
          let rhs_switch_setting =
            Self::get_switch_setting_from_top_bottom_decision(low, permutation.get(high), false);
          switches[right].insert(rhs_switch, rhs_switch_setting);

          let tprime =
            AsWaksmanTopology::calculate_in_out_index(rows_to_generate, low, rhs_switch, false);
      ...
      loop {
        ...
        if should_route_left {
          ...
          let should_use_top =
            Self::get_top_bottom_decision_from_switch_setting(low, to_route, lhs_switch_setting);
          ...
          let should_use_top =
            Self::get_top_bottom_decision_from_switch_setting(low, to_route, rhs_switch_setting);
