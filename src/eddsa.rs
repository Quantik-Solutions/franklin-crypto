...
use hmac::{Hmac, Mac};
use jubjub::{edwards::Point, FixedGenerators, JubjubEngine, JubjubParams, ToUniform, Unknown};
use rand::Rng;
use sha2::{Digest, Sha256};
use std::convert::{TryFrom, TryInto};
use std::io::{self, Read, Write};

use util::{hash_to_scalar, hash_to_scalar_s, rescue_hash_to_scalar, sha256_hash_to_scalar};
...
fn pedersen_h_star<E: JubjubEngine>(bits: &[bool], params: &E::Params) -> E::Fs {
  let result: E::Fr = baby_pedersen_hash::<E, _>(
...
fn rescue_h_star<E: RescueEngine + JubjubEngine>(
  ...
  rescue_hash_to_scalar::<E>(&[], a, b, params)
}
...
impl<E: JubjubEngine> Seed<E> {
  ...
  pub fn deterministic_seed(pk: &PrivateKey<E>, msg: &[u8]) -> Self {
    ...
    pk.0
      .into_repr()
      .write_be(&mut priv_key[..])
      .expect("PK must be representable as bytes slice");
    ...
    loop {
      ...
      s_repr
        .read_be(&k_slice[..])
        .expect("Should be a valid scalar");
...
impl<E: JubjubEngine> PrivateKey<E> {
  ...
  pub fn musig_pedersen_sign(
    ...
    let phash_concatenated: E::Fr =
      baby_pedersen_hash::<E, _>(Personalization::NoteCommitment, concatenated_bits, &params)
        .into_xy()
        .0;
    let mut phash_first_bits: Vec<bool> =
      BitIterator::new(phash_concatenated.into_repr()).collect();
...
impl<E: RescueEngine + JubjubEngine> PublicKey<E> {
  ...
  pub fn verify_musig_rescue(
    ...
    self
      .0
      .mul(c, jubjub_params)
      .add(&sig.r, jubjub_params)
      .add(
        &jubjub_params
          .generator(p_g)
          .mul(sig.s, jubjub_params)
          .negate()
          .into(),
        jubjub_params,
      )
...
mod baby_tests {
  ...
  fn output_signatures_for_rescue_musig() {
    ...
    for msg in vec![
      vec![],
      hex::decode("72").unwrap(),
      hex::decode("af82").unwrap(),
    ]
    .into_iter()
    {
